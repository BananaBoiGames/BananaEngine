function scr_playersounds()
{
    with (obj_player)
    {
        ...
        else if (state != (103 << 0) || !grounded || move == -xscale)
        {
        ...
        if ((state == (121 << 0) || sprite_index == spr_mach3boost) && sprite_index != spr_crazyrun && !audio_is_playing(sfx_mach3))
        {
            mach3snd = audio_play_sound(sfx_mach3, 1, false);
            sfx_gain(mach3snd);
        }
        else if ((state != (121 << 0) && sprite_index != spr_mach3boost) || sprite_index == spr_crazyrun)
        {
